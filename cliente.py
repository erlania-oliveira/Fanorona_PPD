...
class FanoronaClientGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Fanorona - CLIENTE (Jogador 2 - Brancas)")
    ...
    def connect_dialog(self):
        ...
        tk.Label(dialog, text="IP do Servidor:").pack(pady=5)
        ip_entry = tk.Entry(dialog)
        ip_entry.insert(0, "127.0.0.1")
        ip_entry.pack(pady=5)

        tk.Label(dialog, text="Porta:").pack(pady=2)
        port_entry = tk.Entry(dialog, width=8)
        ...
        tk.Button(dialog, text="Conectar", command=connect, bg="#3498db", fg="white").pack(pady=10)
    ...
    def process_network_message(self, msg):
        ...
        if mtype == "MOVE":
            ...
            messagebox.showinfo("Fim de Jogo", "🏳️ Você venceu! O Servidor desistiu da partida.")
    ...
    def update_game_state(self):
        ...
        if x_cnt == 0:
            messagebox.showinfo("Fim de Jogo", "🎉 Você (Brancas) Venceu!")
            self.reset_game(broadcast=False)
            return
        elif o_cnt == 0:
            messagebox.showinfo("Fim de Jogo", "🎉 Servidor (Pretas) Venceu!")
            self.reset_game(broadcast=False)
            return

        is_my_turn = (self.current_player == self.my_symbol)
        msg = f"Você (Brancas) | {'SUA VEZ' if is_my_turn else 'VEZ DO SERVIDOR'} | Pretas: {x_cnt} | Brancas: {o_cnt}"
    ...
    def surrender(self):
        if messagebox.askyesno("Desistir", "Deseja mesmo desistir?"):
            ...
            messagebox.showinfo("Fim de Jogo", "🏳️ Você desistiu. Vitória do Servidor!")
            self.reset_game(broadcast=False)
